...
def make_index(participants: list[str], datasite_path: Path, context=None):
    ...
    for user_folder in participants:
        ...
        if value_file.exists():
            user_level_context = create_context()
            index = index_creator(value_file, target_path=Path(
                datasite_path) / user_folder / "public", context=user_level_context)
...
if __name__ == "__main__":
    embed_model = BgeSmallEmbedModel()
    llm = T5LLM()
    global_context = None  # create_context()
    ...
    make_index(participants, datasite_path, context=None)
